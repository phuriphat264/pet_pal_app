  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: [
        ...
        Provider<ApiClient>(
          create: (_) =>
              ApiClient(authService: authService, client: httpClient),
        ),
        Provider<HotelRepository>(
          create: (ctx) => HotelRepository(ctx.read<ApiClient>()),
        ),
        Provider<PetRepository>(
          create: (ctx) => PetRepository(ctx.read<ApiClient>()),
        ),
        Provider<BookingRepository>(
          create: (ctx) => BookingRepository(ctx.read<ApiClient>()),
        ),
        Provider<PaymentRepository>(
          create: (ctx) => PaymentRepository(ctx.read<ApiClient>()),
        ),
        Provider<PartnerRepository>(
          create: (ctx) => PartnerRepository(
            ctx.read<ApiClient>(),
            uploadClient: httpClient,
          ),
        ),
        Provider<CameraRepository>(
          create: (ctx) => CameraRepository(ctx.read<ApiClient>()),
        ),
        Provider<AdminUserRepository>(
          create: (ctx) => AdminUserRepository(ctx.read<ApiClient>()),
        ),
        Provider<AdminPaymentRepository>(
          create: (ctx) => AdminPaymentRepository(ctx.read<ApiClient>()),
        ),
...
class HomePage extends StatelessWidget {
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        'สวัสดี, คุณ${(context.watch<AuthService>().currentUser?.name ?? "ผู้ใช้").trim().split(' ').first} 👋',
                      ...
                      const Text(
                        'วันนี้น้องของคุณเป็นอย่างไรบ้าง?',
                        overflow: TextOverflow.ellipsis,
                        maxLines: 1,
  ...
  Future<Map<String, dynamic>> _loadHeroData(BuildContext context) async {
    ...
    if (activeBookings.isNotEmpty) {
      ...
      try {
        final cameras = await context
            .read<CameraRepository>()
            .fetchCamerasForHotel(hotelId);
  ...
  Widget _heroCardContent(
    ...
    final titleText = pet != null
        ? '${pet['name']}กำลังนอนหลับ 😴'
        : 'เพิ่มน้องตัวแรกของคุณ 🐾';
                ...
                Column(
                  children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color:
                            (hasCamera ? const Color(0xFF81C784) : Colors.white)
                                .withValues(alpha: 0.25),
                        borderRadius: BorderRadius.circular(10),
                      ),
                      child: Text(
                        hasCamera ? 'ปลอดภัย' : 'ยังไม่เชื่อมต่อ',
                        style: TextStyle(
                          fontSize: 12,
                          color: hasCamera
                  ...
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        boxShadow: [
                          BoxShadow(
                            color: (a['color'] as Color).withValues(
                              alpha: 0.15,
                            ),
                            blurRadius: 8,
                            offset: const Offset(0, 3),
          ...
          const Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 4),
                Text(
                  'รับประกันดูแลน้องด้วยรัก ปลอดภัย 100% พร้อมวงเงินคุ้มครองและวิดีโออัปเดต 24 ชม.',
                  style: TextStyle(
  ...
  Widget _buildNearbySection(BuildContext context) {
    return FutureBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        final nearby = (snapshot.data ?? const <Map<String, dynamic>>[])
            .take(3)
            .toList();
                        ...
                        children: [
                          ...
                          Positioned(
                            ...
                            child: Container(
                              ...
                              child: Text(
                                (h['tags'] as List?)?.isNotEmpty == true
                                    ? (h['tags'] as List).first.toString()
                                    : 'Pet Hotel',
                                style: const TextStyle(
                                  color: Colors.white,
                                  fontSize: 9,
  ...
  Widget _tipCard(BuildContext context, CareTip tip) {
    return GestureDetector(
      onTap: () => Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => CareTipDetailPage(tip: tip)),
      ),
      child: Container(
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(
        ...
        child: Row(
          children: [
            Container(
              ...
              child: CareTipIllustration(type: tip.illustration, size: 40),
            ),
