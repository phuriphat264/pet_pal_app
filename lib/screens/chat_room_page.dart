...
class ChatRoomPage extends StatefulWidget {
  ...
  const ChatRoomPage({
    super.key,
    this.hotelName = 'โรงพยาบาลสัตว์แผ่นดินทอง',
    this.caregiverName = 'พี่เลี้ยงแอน',
...
class _ChatRoomPageState extends State<ChatRoomPage> {
  ...
  late bool _hasActiveBooking;
  final TextEditingController _ctrl = TextEditingController();
  final List<Map<String, dynamic>> _msgs = [
    {'text': 'สวัสดีค่ะ พี่เลี้ยงแอนจาก โรงพยาบาลสัตว์แผ่นดินทอง นะคะ 🐶', 'isMe': false, 'time': '10:00'},
  ...
  void initState() {
    super.initState();
    _hasActiveBooking = widget.hasActiveBooking;
  }
          ...
          children: [
            Text(
              ...
              style: const TextStyle(
                  color: _brown, fontSize: 17, fontWeight: FontWeight.w700),
            ),
            ...
            Row(
              children: [
                ...
                Text(
                  'กำลังออนไลน์ (${widget.caregiverName})',
                  style: const TextStyle(
                      color: _mutedBrown,
                      fontSize: 13,
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('กำลังสลับไปหน้าจอโทรศัพท์...'))),
          ),
          IconButton(
            icon: const Icon(Icons.videocam_rounded, color: _brown, size: 28),
            onPressed: () => ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('กำลังเปิดกล้องวิดีโอคอล...'))),
          ),
                    ...
                    children: [
                      Container(
                        ...
                        child: Text(
                          m['text'] as String,
                          style: TextStyle(
                              color: isMe ? Colors.white : _darkBrown,
                              fontSize: 15,
                              height: 1.4),
                        ),
                      ),
                  ...
                  Expanded(
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        hintStyle:
                            const TextStyle(color: _mutedBrown, fontSize: 14),
                        filled: true,
                        fillColor: _bgCream,
            ...
            child: Column(
              ...
              children: [
                Container(
                  ...
                  height: 120,
                  decoration: BoxDecoration(
                      color: const Color(0xFFE2D0BC).withValues(alpha: 0.3),
                      shape: BoxShape.circle),
                  child: const Icon(Icons.chat_bubble_outline_rounded,
                      size: 50, color: _brown),
                ),
                const SizedBox(height: 24),
                const Text('ยังไม่มีการสนทนา',
                    style: TextStyle(
                        fontSize: 22,
                ...
                const Text(
                  'คุณสามารถแชทพูดคุยกับพี่เลี้ยงเพื่ออัปเดตสถานะของน้องๆ ได้ตลอดเวลา เมื่อทำการจองและเข้าพัก',
                  textAlign: TextAlign.center,
                  style:
                      TextStyle(fontSize: 15, color: _mutedBrown, height: 1.5),
                ),
                ...
                ElevatedButton(
                  ...
                  child: const Text('จองห้องพักเลย',
                      style: TextStyle(
                ...
                TextButton(
                  ...
                  child: const Text('ดูตัวอย่างหน้าแชท (Demo)',
